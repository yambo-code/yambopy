...
def breaking_symmetries(efield1, efield2=[0, 0, 0], folder=".", RmTimeRev=True):
    ...
    ypp = YamboIn.from_runlevel(
        "-y -V all", executable="ypp", folder=folder, filename="ypp.in"
    )
...
def analyse_gw(
    folder, var, bandc, kpointc, bandv, kpointv, pack, text, draw, verbose=False
):
    """
    ...
    for root, dirs, files in os.walk(folder):
        ...
        if any([basename.startswith(v) for v in [var, "reference"]]):
            for filename in files:
                ...
                io[basename] = (
                    YamboIn.from_file(folder=folder, filename="%s.in" % basename),
    ...
    for basename, (inp, out) in io.items():
        ...
        # save result
        qp_gap = (
            eigenvalues_qp[kpointc - out.min_kpoint, bandc - out.min_band]
            - eigenvalues_qp[kpointv - out.min_kpoint, bandv - out.min_band]
        )
    ...
    if convergence_data.dtype == "object":
        raise ValueError("Unknown type of variable")
...
#
def analyse_bse(
    folder, var, numbexc, intexc, degenexc, maxexc, text, draw, verbose=False
):
    """
    ...
    with open(os.path.join(output_folder, output_file), "w") as f:
        ...
        for value, energies in exciton_energies:
            f.write(
                "{} ".format(value)
                + ("%10.6lf " * numbexc) % tuple(energies[:numbexc])
                + "\n"
            )
...
def merge_qp(output, files):
    ...
    try:
        ...
        try:
            qp_test = datasets[0]["QP_E_Eo_Z"]
        except IndexError:
            raise IndexError("Problem with the databases")
        finally:
            merge_qp_compatibility(output, files, verbosity=False)
    finally:
...
def add_qp(output, add=[], substract=[], addimg=[], verbose=False):
    ...
    for d, f in zip(datasets, filenames):
        ...
        """ 
        Description doesn't work, not sure how to fix? 
        Maybe copy from mergeqp but don't want to print the wrong thing!
        - Daniel
        """
        ...
        for (n1, n2, k), (Eo) in zip(QP_table[f], QP_E0[f][:]):
            qpdic[(n1, n2, k)] = Eo
            qpdici[(n1, n2, k)] = 0
...
def plot_excitons(filename, cut=0.2, size=20):
    ...
    def get_var(dictionary, variables):
        ...
        raise ValueError(
            "Could not find the variables %s in the output file" % str(variables)
        )
    ...
    plt.plot(
        get_var(data, ["E/ev", "E/ev[1]"]),
        get_var(data, ["EPS-Im[2]"]),
        label="BSE",
        lw=2,
    )
    plt.plot(
        get_var(data, ["E/ev", "E/ev[1]"]),
        get_var(data, ["EPSo-Im[4]"]),
        label="IP",
        lw=2,
    )
    ...
    for n, exciton in enumerate(sorted_excitons):
        ...
        ax.scatter(
            ...
            cmap=cmap,
            lw=0,
            label="%5.2lf (eV)" % exciton["energy"],
        )
        ax.text(-cut * 0.9, -cut * 0.9, "%5.2lf (eV)" % exciton["energy"])
    ...
    plt.subplots_adjust(
        left=None, bottom=None, right=None, top=None, wspace=0.01, hspace=0.01
    )
