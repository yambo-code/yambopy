...
files = subprocess.check_output('ls */ndb.em1?_fragment_1',shell=True).splitlines()

os.system('mkdir yambo')
os.system('cp yambo_q1/ndb.em1? yambo')
for filename in files:
    q,fname = filename.split('/')
    print q,fname[:-1]+q
    #ugly hack
    q = [s for s in q.split('q') if s.isdigit()][0]

    #open dataset
    ncin = Dataset(filename,'r')

    #new dataset
    ncout = Dataset('yambo/%s'%(fname[:-1]+q),'w',format='NETCDF3_CLASSIC')
