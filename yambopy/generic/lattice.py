...
def car_red(car,lat):
    ...
    """
    return np.array(map( lambda coord: np.linalg.solve(np.array(lat).T,coord), np.array(car)))
...
class Lattice():
    ...
    def set_kmesh(self,kmesh):
        ...
        #irreducible kpoints
        irr_kpoints  = (grid[counter.keys()] + [0.0, 0.0, 0.0]) / kmesh
        full_kpoints = (grid + [0.0, 0.0, 0.0]) / kmesh
        mapping_to_ibz = dict(zip(counter.keys(),xrange(nkpoints_ibz)))
    ...
    def get_wigner(self,mesh,eps=1e-7):
        ...
        yiter = xrange(-ny,ny)
        ziter = xrange(-nz,nz)
        if nx == 1: xiter = xrange(1)
        if ny == 1: yiter = xrange(1)
        if nz == 1: ziter = xrange(1)
        ...
        neighbours = []
        for x,y,z in product(xrange(-1,2),repeat=3):
            #do not add zero
    ...
    def get_double_grid(self,lattice,debug=False,delta_shift=[1e-4,1e-4,1e-4]):
            ...
            coarse_kpoints = []
            coarse_indexes = []
            for i,j,k in itertools.product(xrange(-1,2),repeat=3):
                disp = red_car([[i,j,k]],self.rlat)[0]
                coarse_kpoints.append( self.car_kpoints + disp )
                coarse_indexes.append( xrange(self.full_nkpoints) )
            ...
            coarse_to_double_grid = [ [] for nk in xrange(self.full_nkpoints) ] #one to many
