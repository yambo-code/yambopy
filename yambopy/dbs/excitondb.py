...
class YamboExcitonDB(YamboSaveDB):
    ...
    @classmethod
    def from_db_file(cls,lattice,filename='ndb.BS_PAR_Q1',folder='.'):
        """ initialize this class from a file
        """
        path_filename = os.path.join(folder,filename)
        if not os.path.isfile(path_filename):
        ...
        with Dataset(path_filename) as database:
            if 'BSE_RESONANT' in database.variables:
                ker = database.variables['BSE_RESONANT'][:]
                ker = ker[:,:,0] + ker[:,:,1]*I
                kernel = ker

        return cls(lattice,kernel)

    def get_kernel_value(q,excitons,yker,yexc):
        """get value of the kernel weighted by selected exciton wf"""
        """K = sum_{kcv,kkccvv}(A^l_{kkccvv}*A^l_{kcv}<kkccvv|K|kcv>)"""
        """K = sum_{t,tt}(A^l_{tt}*A^l_{t}<tt|K|t>)"""
        """where t->kcv according to table"""
        # Initialize the value of the kernel to zero
        K  = np.zeros(len(excitons),dtype=np.complex_)
        Kr = np.zeros((1,2*len(excitons)+1),dtype=np.float_)
        Kr[0,0] = q

        # Generate a square matrix from the lower triangular one
        z = np.transpose(np.tril(yker.kernel.data))+np.tril(yker.kernel.data)
        ...
        for exciton in excitons:
            #get the eigenstate
            eivec = yexc.eigenvectors[exciton-1]

            #compute avarage kernel
            for t,kcv in enumerate(yexc.table):
                for tt,kkccvv in enumerate(yexc.table):
                    K[exciton-1] += np.vdot(yexc.eigenvectors[exciton-1,tt],yexc.eigenvectors[exciton-1,t])*z[tt,t]
            Kr[0,2*(exciton-1)+1] = K[exciton-1].real
            Kr[0,2*exciton]   = K[exciton-1].imag

 
        return Kr

    def get_kernel_value_bands(size,bands,yker,yexc):
        """get value of the kernel for specified set of bands"""
        ...
        W = np.zeros((size,size),dtype=np.complex)

        #compute avarage kernel
        for t,kcv in enumerate(yexc.table):
            k,c,v,flag1,flag2 = kcv
            if c==bands[0] and v==bands[1] :
                for tt,kkccvv in enumerate(yexc.table):
                    kk,cc,vv,fflag1,fflag2 = kkccvv
                    if cc==bands[0] and vv==bands[1] :
                        W[k-1,kk-1] = z[tt,t]
