...
class YamboQPDB():
    ...
    def interpolate(self,lattice,path,what='QP+KS',lpratio=5,valence=None,verbose=1,**kwargs):
        ...
        if 'KS' in what:
            ...
            if valence: kwargs['fermie'] = np.max(dft_eigens_kpath[0,:,:valence])
            ks_ebands = YambopyBandStructure(dft_eigens_kpath[0],kpoints_path,kpath=path,**kwargs)
        ...
        if 'QP' in what:
            ...
            if valence: kwargs['fermie'] = np.max(qp_eigens_kpath[0,:,:valence])
            qp_ebands = YambopyBandStructure(qp_eigens_kpath[0],kpoints_path,kpath=path,**kwargs)
