...
# import os
# from yambopy.common.precision import *
...
#         self.numpy_float = self.lelph_db.ph_energies.dtype
#         self.numpy_complex = self.lelph_db.ph_eigenvectors.dtype
...
#                 self.ydipdb = ydipdb
#             else :
#                 self.ydipdb = YamboDipolesDB(self.ydb, filename = ndb_dipoles_fname, dip_type='iR',\
...
#         # (inv_blat) = (ydb.lat.T)
#         sym_red = np.linalg.inv(self.ydb.lat.T) @ symm_mats.T @ self.ydb.lat.T
...
#         ome_range = np.linspace(self.ome_range[0], self.ome_range[1], num=self.ome_range[2])
...
#         print(
#             f'Minimum energy of the Direct exciton is : {min(self.BS_eigs[0])*27.2114:.4f} eV'
#         )
...
#             inte_tmp = self.compute_luminescence_per_freq(ome_range[i], self.ph_freq.data, exe_ene, \
#                 Exe_min, self.ex_dip, self.ex_ph, npol=self.npol)
...
#     @njit(cache=True, nogil=True, parallel=True)
#     def compute_luminescence_per_freq(self,ome_light,
#                             ph_freq,
#                             ex_ene,
#                             exe_low_energy,
#                             ex_dip,
...
#         broading = self.numpy_float(broading / 27.211 / 2)
#         ome_light_Ha = self.numpy_float(ome_light / 27.211)
#         KbT = self.numpy_float(3.1726919127302026e-06 * temp)  ## Ha
...
#                 Tmu = np.zeros((npol, nbnd_f), dtype=self.numpy_complex)  # D*G
#                 ## compute scattering matrix
#                 for ipol in range(npol):
#                     for ii in range(nbnd_i):
#                         Tmu[ipol,:] = Tmu[ipol,:] + np.conj(ex_ph[iq,iv,ii,:]) * ex_dip[ipol,ii] \
#                             /(ex_ene[0,ii] - E_f_omega + self.numpy_complex(1j*broading))
